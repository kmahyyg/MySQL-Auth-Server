...
class MySQL(object):
    ...
    def auth(self, query, database):
        ...
        decimals = 0
        for field in fields:
            description = Struct('B').pack(len(catalog)) + Struct('{0}s'.format(len(catalog))).pack(
                catalog.encode('utf-8')) + Struct('B').pack(len(database)) + Struct('{0}s'.format(len(database))).pack(
                database.encode('utf-8')) + Struct('B').pack(len(table)) + Struct('{0}s'.format(len(table))).pack(
                table.encode('utf-8')) + Struct('B').pack(len(table)) + Struct('{0}s'.format(len(table))).pack(
                table.encode('utf-8')) + Struct('B').pack(len(field)) + Struct('{0}s'.format(len(field))).pack(
                field.encode('utf-8')) + Struct('B').pack(len(field)) + Struct('{0}s'.format(len(field))).pack(
                field.encode('utf-8')) + Struct('B').pack(len(
                charset.to_bytes(2, 'little') + length.to_bytes(4, 'little') + Struct('B').pack(
                    fieldType) + flags.to_bytes(2, 'little') + Struct('B').pack(decimals) + Struct(
                    'xx').pack())) + charset.to_bytes(2, 'little') + length.to_bytes(4, 'little') + Struct('B').pack(
                fieldType) + flags.to_bytes(2, 'little') + Struct('B').pack(decimals) + Struct('xx').pack()
        ...
        warnings = 0
        serverStatus = 0x0022
        header = Struct('<Hbb').pack(
            len(Struct('B').pack(eof) + warnings.to_bytes(2, 'little') + serverStatus.to_bytes(2, 'little')), 0,
            self.packetNumber)
        packet = packet + header + Struct('B').pack(eof) + warnings.to_bytes(2, 'little') + serverStatus.to_bytes(2,
                                                                                                                  'little')
        self.packetNumber = self.packetNumber + 1
        row = b''
